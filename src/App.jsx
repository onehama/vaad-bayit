    ...
    <div dir="rtl" style={{ "--f": "'Noto Sans Hebrew', sans-serif", minHeight: "100vh", background: "linear-gradient(160deg,#1a2744 0%,#2d4a7a 50%,#1a2744 100%)", fontFamily: "var(--f)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      {step === "role" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14, width: "100%", maxWidth: 340 }}>
          <p style={{ textAlign: "center", color: "rgba(255,255,255,0.6)", fontSize: 14, margin: "0 0 4px" }}>בחר/י סוג משתמש</p>
          <button onClick={() => setStep("pickCommittee")}
            style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(10px)", border: "2px solid rgba(196,168,130,0.4)", borderRadius: 20, padding: "22px 20px", cursor: "pointer", textAlign: "right", transition: "all 0.2s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 50, height: 50, borderRadius: 14, background: "linear-gradient(135deg,#c4a882,#d4bc9a)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>⚙️</div>
              <div><div style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: "var(--f)" }}>חבר/ת ועד</div><div style={{ fontSize: 12, color: "#c4a882", marginTop: 2, fontFamily: "var(--f)" }}>ניהול מלא · תשלומים · החלטות · דיירים</div></div>
            </div>
          </button>
          <button onClick={() => setStep("pickResident")}
            style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(10px)", border: "2px solid rgba(255,255,255,0.15)", borderRadius: 20, padding: "22px 20px", cursor: "pointer", textAlign: "right", transition: "all 0.2s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 50, height: 50, borderRadius: 14, background: "linear-gradient(135deg,#4a7a9a,#5a8ab0)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>🏠</div>
              <div><div style={{ fontSize: 16, fontWeight: 700, color: "#fff", fontFamily: "var(--f)" }}>דייר/ת</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 2, fontFamily: "var(--f)" }}>צפייה · חתימה · מצב תשלומים</div></div>
            </div>
          </button>
        </div>
      )}
      {step === "pickCommittee" && (
        <div style={{ width: "100%", maxWidth: 340 }}>
          <button onClick={() => setStep("role")} style={{ background: "none", border: "none", color: "#c4a882", fontSize: 13, cursor: "pointer", fontFamily: "var(--f)", fontWeight: 600, padding: 0, marginBottom: 14 }}>→ חזרה</button>
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 14, margin: "0 0 12px" }}>בחר/י חבר ועד:</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {RESIDENTS.filter((r) => r.isCommittee).map((r) => {
              const ent = ENTRANCES.find((e) => e.id === r.entrance);
              return (
                <button key={r.id} onClick={() => onLogin(r.id, "committee")}
                  style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(196,168,130,0.3)", borderRadius: 14, padding: "14px 16px", cursor: "pointer", textAlign: "right", display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(135deg,#c4a882,#d4bc9a)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "#1a2744" }}>{r.name.charAt(r.name.indexOf(" ") + 1)}</div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600, color: "#fff", fontFamily: "var(--f)" }}>{r.name}</div><div style={{ fontSize: 11, color: "#c4a882", fontFamily: "var(--f)" }}>{r.role} · {ent.label} · דירה {r.apt}</div></div>
                </button>
              );
            })}
          </div>
      ...
      {step === "pickResident" && (
        <div style={{ width: "100%", maxWidth: 340 }}>
          <button onClick={() => { setStep("role"); setSelectedEntrance(null); }} style={{ background: "none", border: "none", color: "#c4a882", fontSize: 13, cursor: "pointer", fontFamily: "var(--f)", fontWeight: 600, padding: 0, marginBottom: 14 }}>→ חזרה</button>
          {!selectedEntrance ? (
            <>
              <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 14, margin: "0 0 12px" }}>בחר/י כניסה:</p>
              <div style={{ display: "flex", gap: 10 }}>
                {ENTRANCES.map((e) => (
                  <button key={e.id} onClick={() => setSelectedEntrance(e.id)}
                    style={{ flex: 1, background: e.color + "30", border: `2px solid ${e.color}60`, borderRadius: 16, padding: "20px 10px", cursor: "pointer", textAlign: "center" }}>
                    <div style={{ fontSize: 28, fontWeight: 800, color: "#fff", fontFamily: "var(--f)" }}>{e.id}</div>
                    <div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", marginTop: 4, fontFamily: "var(--f)" }}>{e.label}</div>
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              <button onClick={() => setSelectedEntrance(null)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 12, cursor: "pointer", fontFamily: "var(--f)", padding: 0, marginBottom: 10 }}>← החלף כניסה</button>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {RESIDENTS.filter((r) => r.entrance === selectedEntrance).map((r) => (
                  <button key={r.id} onClick={() => onLogin(r.id, "resident")}
                    style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 14, padding: "14px 16px", cursor: "pointer", textAlign: "right", display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: "#fff" }}>{r.apt}</div>
                    <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600, color: "#fff", fontFamily: "var(--f)" }}>{r.name}</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", fontFamily: "var(--f)" }}>דירה {r.apt} · קומה {r.floor}</div></div>
                  </button>
                ))}
              </div>
            </>
